...
data_train = np.load('../data_prep/_train.npy')
data_validation = np.load('../data_prep/_validation.npy')

# Data Preprocessing: Normalize data.

X_train = np.concatenate(data_train[:,0])[:, :, :40] #[:,0:128,:]
X_train = np.concatenate(data_train[:,0])
...
y_train = np.repeat(y_train, X_train.shape[1], axis=-1)[:,:,np.newaxis]
...
#ross = RandomOverSampler(random_state=0)
#X_train, y_train = ross.fit_resample(X_train, y_train)
...
tensorboard = keras.callbacks.TensorBoard(
    log_dir='./logscnn_40feats_single_learnable_norm',
    histogram_freq=1,
